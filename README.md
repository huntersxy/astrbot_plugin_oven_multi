...
| `max_universal_inject` | 注入 system prompt 的最大通用风格条数 | `5` |
...
插件提供 Dashboard 页面，可在 AstrBot 管理面板中访问，包含以下内容：

- **风格学习**：查看每个群组的通用风格特征和聊天记录
- **余额查询**：查看各服务商余额信息
...
### 风格学习
插件自动收集聊天记录，默认每 6 小时调用 LLM 分析群聊的总体说话风格，将学习到的通用风格按熟练度排序后注入到 LLM 的 system prompt 中。
